                ...
                while($donnee = $rep1->fetch()){
                    if($donnee['email'] == $_SESSION['online']){
                        ?>
                            ...
                            <input type="text" name="nom" id="prenom" value="<?php echo $donnee['nom_users'] ?>" required>
                            <input type="text" name="prenom" id="prenom" value="<?php echo $donnee['prenom_users']?>"required>
                            <input type="email" name="email" id="prenom" value="<?php echo $donnee['email']?>"required>
                            <input type="text" name="num" id="prenom" value="<?php echo $donnee['num_bancaire']?>"required>
                            <label for="">Bio</label>
                            <textarea name="bio" id="" rows="2"><?php echo $donnee['bio']?></textarea>
                            <label for="">Photo de profil</label>
                            <input type="file" name='profil' id="" value="<?php echo $donnee['photo_users']?>" required>
                        <?php
                    }
                } 
    ?>
